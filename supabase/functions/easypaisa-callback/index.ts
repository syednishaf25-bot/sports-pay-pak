...
serve(async (req) => {
  ...
  try {
    ...
    const paymentStatus = pp_ResponseCode === '0000' ? 'completed' : 'failed'
    const orderStatus = pp_ResponseCode === '0000' ? 'paid' : 'cancelled'
    ...
    if (pp_ResponseCode === '0000') {
      console.log(`Order ${pp_TxnRefNo} paid successfully via EasyPaisa`)
    }

    // Redirect user based on payment result
    const redirectUrl = pp_ResponseCode === '0000' 
